...
class MemoryConsolidatorTest {
    ...
    void maybeRun_skipsWhenTimeGateNotMet(@TempDir Path dir) throws IOException {
        Path memDir = dir.resolve(".codeflow/memory");
    ...
    void maybeRun_skipsWhenSessionGateNotMet(@TempDir Path dir) throws IOException {
        Path memDir = dir.resolve(".codeflow/memory");
        Path sessDir = dir.resolve(".codeflow/sessions");
        Files.createDirectories(memDir);
    ...
    @Test
    @EnabledIfEnvironmentVariable(named = "CODEFLOW_TEST_API_KEY", matches = ".+")
    void e2e_consolidationMergesDuplicates(@TempDir Path dir) throws Exception {
        String apiKey = System.getenv("CODEFLOW_TEST_API_KEY");
        String baseUrl = System.getenv().getOrDefault("CODEFLOW_TEST_BASE_URL", "https://api.minimaxi.com/v1");
        String model = System.getenv().getOrDefault("CODEFLOW_TEST_MODEL", "MiniMax-M3");
