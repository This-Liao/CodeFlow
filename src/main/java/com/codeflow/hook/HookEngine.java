...
public class HookEngine {
     ...
     * 注入 CODEFLOW_EVENT、CODEFLOW_TOOL、CODEFLOW_FILE_PATH 环境变量。
     * 支持超时保护，超时后强制终止子进程。
     */
    private HookResult executeCommand(Hook h, HookContext ctx) {
        ...
        try {
            ...
            // 注入环境变量
            env.put("CODEFLOW_EVENT", ctx.event() != null ? ctx.event().value() : "");
            env.put("CODEFLOW_TOOL", ctx.toolName() != null ? ctx.toolName() : "");
            env.put("CODEFLOW_FILE_PATH", ctx.filePath() != null ? ctx.filePath() : "");
