...
/**
 * 加载项目和用户级别的指令文件（CODEFLOW.md / AGENTS.md），支持 @include 递归展开。
 *
 * <p>发现顺序（越靠后优先级越高，模型注意力优先关注后出现的内容）：
 * <ol>
 *   <li>用户全局：~/.codeflow/CODEFLOW.md、~/.codeflow/AGENTS.md</li>
 *   <li>项目级：从 git root 沿目录树向下到 workDir，每个目录的 CODEFLOW.md 和 AGENTS.md</li>
 *   <li>workDir/.codeflow/INSTRUCTIONS.md</li>
 *   <li>workDir/CODEFLOW.local.md（私有本地覆盖）</li>
 * </ol>
 *
 * <p>@include 指令：
 * <ul>
...
public final class InstructionLoader {
    ...
    static List<InstructionSource> discoverInstructions(String workDir) {
        ...
        if (home != null && !home.isEmpty()) {
            addSource(sources, seen, Path.of(home, ".codeflow", "CODEFLOW.md"));
            addSource(sources, seen, Path.of(home, ".codeflow", "AGENTS.md"));
        }

        // 2. 项目级：从 git root 到 workDir 的每一级目录
        for (String dir : projectInstructionDirs(workDir)) {
            addSource(sources, seen, Path.of(dir, "CODEFLOW.md"));
            addSource(sources, seen, Path.of(dir, "AGENTS.md"));
        }

        // 3. 项目内通用指令
        addSource(sources, seen, Path.of(workDir, ".codeflow", "INSTRUCTIONS.md"));

        // 4. 本地私有覆盖
        addSource(sources, seen, Path.of(workDir, "CODEFLOW.local.md"));
