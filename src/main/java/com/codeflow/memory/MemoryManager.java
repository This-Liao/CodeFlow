 ...
 * <p>存储结构：
 * <ul>
 *   <li>用户级 (~/.codeflow/memory/)：存放 type=user / type=feedback 的记忆文件</li>
 *   <li>项目级 (.codeflow/memory/)：存放 type=project / type=reference 的记忆文件</li>
 * </ul>
 *
...
public class MemoryManager {
    ...
    private static final String MEMORY_DIR = ".codeflow/memory";
    ...
    /** 返回用户级记忆目录（~/.codeflow/memory/） */
    public Path userMemDir() {
        return userMemDirPath;
    }

    /** 返回项目级记忆目录（.codeflow/memory/） */
    ...
    /**
     * 加载指令文件：支持用户级（~/.codeflow/CODEFLOW.md）、项目级（git root 到 workDir 逐层）、
     * INSTRUCTIONS.md、私有 CODEFLOW.local.md，以及 @include 递归展开。
