...
public class AgentTool implements Tool {
    ...
    private com.codeflow.worktree.WorktreeManager worktreeManager;

    /** Optional: team manager for team_name registration. */
    private com.codeflow.teams.TeamManager teamManager;
    ...
    public void setWorktreeManager(com.codeflow.worktree.WorktreeManager worktreeManager) {
        this.worktreeManager = worktreeManager;
    }

    public void setTeamManager(com.codeflow.teams.TeamManager teamManager) {
        ...
        for (var msg : parent.getMessages()) {
            if (msg.getToolUses() != null && !msg.getToolUses().isEmpty()
                ...
                var placeholders = msg.getToolUses().stream()
                        .map(tu -> new com.codeflow.conversation.ToolResultBlock(
    ...
    private ToolResult runSync(SubAgentSpec spec, String description, String prompt, String modelOverride, String isolation) {
        ...
        com.codeflow.worktree.AgentWorktree.Result wtResult = null;
        if ("worktree".equals(isolation) && worktreeManager != null) {
            ...
            try {
                wtResult = com.codeflow.worktree.AgentWorktree.create(
                ...
                String notice = com.codeflow.worktree.AgentWorktree.buildNotice(
        ...
        while (true) {
            ...
            switch (event) {
                ...
                case AgentEvent.LoopComplete lc -> {
                    ...
                    String wtInfo = "";
                    if (wtResult != null) {
                        if (com.codeflow.worktree.WorktreeChanges.hasChanges(
                            ...
                            com.codeflow.worktree.AgentWorktree.remove(
    ...
    private ToolResult runAsTeammate(SubAgentSpec spec, String teamName,
        ...
        if (team == null) {
            team = teamManager.createTeam(teamName, com.codeflow.teams.TeamManager.detectBackend(),
                    com.codeflow.teams.TeammateRunner.LEAD_NAME, description);
        }
        ...
        subRegistry.register(new com.codeflow.teams.TeamTools.SendMessageTool(teamManager, memberName));
        // 团队共享任务板工具：队友之间共享同一份任务列表（覆盖个人版同名工具）
        subRegistry.register(new com.codeflow.teams.TeamTaskTools.TaskCreateTool(teamManager, teamName, memberName));
        subRegistry.register(new com.codeflow.teams.TeamTaskTools.TaskGetTool(teamManager, teamName));
        subRegistry.register(new com.codeflow.teams.TeamTaskTools.TaskListTool(teamManager, teamName));
        subRegistry.register(new com.codeflow.teams.TeamTaskTools.TaskUpdateTool(teamManager, teamName));
        ...
        // Build addendum
        String addendum = com.codeflow.teams.TeammateRunner.buildTeammateAddendum(
        ...
        if ("worktree".equals(isolation) && worktreeManager != null) {
            try {
                ...
                new java.security.SecureRandom().nextBytes(rndBytes);
                String slug = "agent-a" + java.util.HexFormat.of().formatHex(rndBytes).substring(0, 7);
                var wtResult = com.codeflow.worktree.AgentWorktree.create(
                        slug, worktreeManager.getProjectRoot(), worktreeManager.getSymlinkDirs());
        ...
        // Spawn teammate
        try {
            var spawnResult = com.codeflow.teams.SpawnDispatcher.spawnTeammate(
                    new com.codeflow.teams.SpawnDispatcher.SpawnConfig(
     ...
     */
    private com.codeflow.permission.PermissionChecker teammateChecker(boolean planModeRequired) {
        if (!planModeRequired) {
            // 普通队友不带检查器，沿用既有行为
            return null;
        }
        var root = java.nio.file.Path.of(System.getProperty("user.dir"));
        return new com.codeflow.permission.PermissionChecker(
                com.codeflow.permission.PermissionMode.PLAN, root);
    }
