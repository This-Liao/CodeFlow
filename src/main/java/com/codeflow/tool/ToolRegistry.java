...
public class ToolRegistry {
    ...
    private volatile Set<String> contextOmittedTools = Set.of();
    ...
    public List<String> getDeferredToolNames() {
        return tools.values().stream()
                .filter(t -> (t.shouldDefer() || contextOmittedTools.contains(t.name()))
                        && !discoveredTools.contains(t.name()))
                .map(Tool::name)
                .toList();
    }

    public void setContextOmittedTools(Set<String> names) {
        contextOmittedTools = names == null ? Set.of() : Set.copyOf(names);
    }

    public Set<String> getContextOmittedTools() { return contextOmittedTools; }
    ...
    public List<Map<String, Object>> getAllSchemas(String protocol) {
        ...
        for (var tool : tools.values()) {
            if ((tool.shouldDefer() || contextOmittedTools.contains(tool.name()))
                    && !discoveredTools.contains(tool.name())) continue;
    ...
    public List<Map<String, Object>> searchDeferred(String query, int maxResults, String protocol) {
        ...
        for (var tool : tools.values()) {
            if (!(tool.shouldDefer() || contextOmittedTools.contains(tool.name()))) continue;
    ...
    public static ToolRegistry createDefault() {
        ...
        reg.register(new com.codeflow.tool.impl.ReadFileTool());
        reg.register(new com.codeflow.tool.impl.WriteFileTool());
        reg.register(new com.codeflow.tool.impl.EditFileTool());
        reg.register(new com.codeflow.tool.impl.BashTool());
        reg.register(new com.codeflow.tool.impl.GlobTool());
        reg.register(new com.codeflow.tool.impl.GrepTool());
        return reg;
    }
}
