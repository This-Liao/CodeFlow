...
import com.codeflow.config.ConfigLoader;
import com.codeflow.config.McpServerConfig;
import com.codeflow.config.ProviderConfig;
import com.codeflow.llm.LlmClient;
import com.codeflow.print.PrintMode;
import com.codeflow.prompt.PromptBuilder;
import com.codeflow.remote.RemoteServer;
import com.codeflow.teams.TeamManager;
import com.codeflow.teams.TeamTools;
import com.codeflow.teams.TeammateRunner;
import com.codeflow.tool.ToolRegistry;
import com.codeflow.tool.impl.BashTool;
import com.codeflow.tool.impl.EditFileTool;
import com.codeflow.tool.impl.GlobTool;
import com.codeflow.tool.impl.GrepTool;
import com.codeflow.tool.impl.ReadFileTool;
import com.codeflow.tool.impl.WriteFileTool;
import com.codeflow.tui.CodeFlowModel;
...
public class CodeFlow {
    ...
    private static final String CONFIG_ENV = "CODEFLOW_CONFIG";

    public static void main(String[] args) {
        // 队友进程入口：被 tmux/iTerm 拉起时命令以 --teammate 打头，走队友模式而非 TUI。
        // 格式：codeflow --teammate --team-name <t> --agent-name <n>（见 SpawnDispatcher.buildTeammateCLI）
        ...
        for (int i = 0; i < args.length; i++) {
            ...
            } else if (arg.equals("--eval-report") && i + 1 < args.length) {
                evalTracePath = args[++i];
            } else if (arg.startsWith("--eval-report=")) {
                evalTracePath = arg.substring("--eval-report=".length());
            } else if (arg.equals("--baseline-report") && i + 1 < args.length) {
                baselineReportPath = args[++i];
            } else if (arg.startsWith("--baseline-report=")) {
                baselineReportPath = arg.substring("--baseline-report=".length());
            } else if (arg.equals("--fail-on-regression")) {
                failOnRegression = true;
            } else if (arg.equals("--durable")) {
                durableRun = true;
            } else if (arg.equals("--resume-task") && i + 1 < args.length) {
                resumeTaskId = args[++i];
                durableRun = true;
            } else if (arg.startsWith("--resume-task=")) {
                resumeTaskId = arg.substring("--resume-task=".length());
                durableRun = true;
        ...
        if (evalTracePath != null) {
            var output = com.codeflow.eval.EvalReportGenerator.write(
                    ...
                    baselineReportPath == null ? null : java.nio.file.Path.of(baselineReportPath),
                    java.nio.file.Path.of(System.getProperty("user.dir"), ".codeflow", "reports", "latest"));
            System.out.println("Eval report: " + output.markdown().toAbsolutePath());
            System.out.println("Regression gate: " + (output.report().passedRegressionGate() ? "PASS" : "FAIL"));
            if (failOnRegression && !output.report().passedRegressionGate()) System.exit(2);
            return;
        }

        // 环境变量回退
        if (configPath == null) {
            String envPath = configuredPath();
        ...
        if (printPrompt != null) {
            ...
            PrintMode.run(config, printPrompt, fmt,
                    new PrintMode.DurableOptions(durableRun, resumeTaskId));
            return;
        }
    ...
    private static void runTeammate(String[] args) {
        ...
        // 加载与 TUI 相同的配置（默认路径 / CODEFLOW_CONFIG 环境变量）
        AppConfig config;
        try {
            config = ConfigLoader.load(configuredPath());
        } catch (ConfigLoader.ConfigException e) {
            System.err.println("Configuration error: " + e.getMessage());
            System.exit(1);
            return;
        ...
        // 工具上供模型按名字加载
        var skillCatalog = com.codeflow.skill.SkillCatalog.loadCatalog(workDir);
    ...
    private static String configuredPath() {
        String current = System.getenv(CONFIG_ENV);
    ...
    static ToolRegistry buildTeammateRegistry(
        ...
        registry.register(new com.codeflow.tool.impl.ToolSearchTool(registry, protocol));
        registry.register(new com.codeflow.tool.SyntheticOutputTool());

        var worktreeManager = new com.codeflow.worktree.WorktreeManager(workDir, List.of(), 720);
        ...
        registry.register(new com.codeflow.teams.TeamTaskTools.TaskCreateTool(teamMgr, teamName, memberName));
        registry.register(new com.codeflow.teams.TeamTaskTools.TaskGetTool(teamMgr, teamName));
        registry.register(new com.codeflow.teams.TeamTaskTools.TaskListTool(teamMgr, teamName));
        registry.register(new com.codeflow.teams.TeamTaskTools.TaskUpdateTool(teamMgr, teamName));

        if (mcpConfigs != null && !mcpConfigs.isEmpty()) {
            try {
                var result = new com.codeflow.mcp.McpManager(mcpConfigs).connectAll();
