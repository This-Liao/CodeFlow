...
public final class SpawnDispatcher {
    ...
    public record SpawnConfig(
            ...
            String protocol,
            com.codeflow.config.ProviderConfig providerConfig,
            String workdir,
            /**
             * 队友的权限检查器。Lead 派人时标了 plan_mode_required，
             * 这里就是一个 PLAN 模式的 checker，队友只能读不能改，直到计划获批。
             */
            com.codeflow.permission.PermissionChecker checker
    ) {}
     ...
     * Format: cd '<workdir>' && '<codeflow>' --teammate --team-name <t> --agent-name <n>
     */
    public static String buildTeammateCLI(String teamName, String memberName, String workdir) {
        ...
        // Find codeflow executable (assume it's the current JAR or on PATH)
        String codeflow = ProcessHandle.current().info().command().orElse("codeflow");
        return "cd %s && %s --teammate --team-name %s --agent-name %s".formatted(
                shellQuote(wd), shellQuote(codeflow), shellQuote(teamName), shellQuote(memberName));
    }
