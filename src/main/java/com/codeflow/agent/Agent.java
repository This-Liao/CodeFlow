...
import com.codeflow.conversation.ThinkingBlock;
import com.codeflow.conversation.ToolResultBlock;
import com.codeflow.conversation.ToolUseBlock;
...
public class Agent {
    ...
    private java.util.function.Consumer<AgentEvent> eventObserver = ignored -> {};
    private com.codeflow.context.ContextPolicy contextPolicy;
    /**
    ...
    private final com.codeflow.compact.ContextCompactor.AutoCompactTrackingState compactTracking =
            new com.codeflow.compact.ContextCompactor.AutoCompactTrackingState();
     ...
     */
    private final com.codeflow.compact.RecoveryState recoveryState =
            new com.codeflow.compact.RecoveryState();

    public com.codeflow.compact.RecoveryState getRecoveryState() { return recoveryState; }

    private com.codeflow.filehistory.FileHistory fileHistory;
    public void setFileHistory(com.codeflow.filehistory.FileHistory fh) { this.fileHistory = fh; }
    public com.codeflow.filehistory.FileHistory getFileHistory() { return fileHistory; }
    ...
    public void setEventObserver(java.util.function.Consumer<AgentEvent> observer) {
        this.eventObserver = observer == null ? ignored -> {} : observer;
    }
    public void setContextPolicy(com.codeflow.context.ContextPolicy contextPolicy) {
    ...
    private void agentLoop(ConversationManager conv, BlockingQueue<AgentEvent> queue) {
        ...
        for (int iteration = 1; ; iteration++) {
            ...
            if (contextPolicy != null) contextPolicy.apply(registry, protocol, conv);
            ...
            try {
                ...
                String compactMsg = com.codeflow.compact.ContextCompactor.manage(
                ...
                if (lastErr != null && (lastErr.contains("context") || lastErr.contains("too long")
                    ...
                    if (contextRetries < 3) {
                        ...
                        try {
                            ...
                            com.codeflow.compact.ContextCompactor.forceCompact(
            ...
            for (var r : results) {
                ...
                if (content.length() > com.codeflow.tool.ToolRegistry.MAX_OUTPUT_CHARS
    ...
    private void putSafe(BlockingQueue<AgentEvent> queue, AgentEvent event) {
        observeEvent(event);
        try {
            queue.put(event);
        } catch (InterruptedException e) {
    ...
    private void observeEvent(AgentEvent event) {
        try {
    ...
    private void persistLastMessage(ConversationManager conv) {
        ...
        com.codeflow.session.SessionManager.saveConversationMessage(
