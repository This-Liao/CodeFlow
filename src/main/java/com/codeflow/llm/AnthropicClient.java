...
public class AnthropicClient implements LlmClient {
    ...
    public AnthropicClient(ProviderConfig cfg, String systemPrompt) {
        ...
        if (apiKey.isEmpty()) {
            throw new LlmException.AuthenticationException(
                    "Anthropic API key not found. Set it in .codeflow/config.yaml or via ANTHROPIC_API_KEY env var.");
        }
    ...
    private void doStream(ConversationManager conv, List<Map<String, Object>> tools,
        ...
        var messageParams = buildMessages(com.codeflow.conversation.ToolPairing.ensure(conv.getMessages()));
