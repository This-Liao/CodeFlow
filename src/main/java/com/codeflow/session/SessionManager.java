...
public class SessionManager {
    ...
    private static Path sessionsDir(String workDir) {
        return Path.of(workDir, ".codeflow", "sessions");
    }
    ...
    public static void saveConversationMessage(String workDir, String sessionId,
                                               com.codeflow.conversation.Message msg) {
        saveRecord(workDir, sessionId, msg.getRole(), null, msg.getContent(),
                toolUseRecords(msg), toolResultRecords(msg));
    }

    private static List<ToolUseRecord> toolUseRecords(com.codeflow.conversation.Message msg) {
    ...
    private static List<ToolResultRecord> toolResultRecords(com.codeflow.conversation.Message msg) {
    ...
    private static ConversationManager replay(List<SessionMessage> messages) {
        ...
        for (SessionMessage msg : messages) {
            ...
            com.codeflow.conversation.Message restored = toConversationMessage(msg);
    ...
    public static com.codeflow.conversation.Message toConversationMessage(SessionMessage msg) {
        List<com.codeflow.conversation.ToolUseBlock> uses = new ArrayList<>();
        for (ToolUseRecord tu : msg.toolUses()) {
            uses.add(new com.codeflow.conversation.ToolUseBlock(
                    tu.toolUseId(), tu.toolName(), tu.arguments()));
        }
        List<com.codeflow.conversation.ToolResultBlock> results = new ArrayList<>();
        for (ToolResultRecord tr : msg.toolResults()) {
            results.add(new com.codeflow.conversation.ToolResultBlock(
                    tr.toolUseId(), tr.content(), tr.isError()));
        }
        var restored = new com.codeflow.conversation.Message(msg.role(), msg.content());
    ...
    public static KeepMessage toKeepMessage(com.codeflow.conversation.Message msg) {
